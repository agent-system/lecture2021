// 0414 サラマンダー水球_青の方のコントローラ。soccerのデモを参考に。
/*
...
int main() {
  ...
  while (wb_robot_step(CONTROL_STEP) != -1) {
    ...
    while (wb_receiver_get_queue_length(receiver) > 0) {
      ...
      //ボール
      ballX = message[9], ballZ = message[10];
    ...
    if (control == AUTO) {
      ...
      if(myX - ballX < 0) { //ボールよりある程度左にいる
        ...
        }else{ //ボールと重なったXの範囲か、右にいる
        //このときはとにかく左に行く。途中でボールを蹴飛ばしても知ったことではない
        // 0が上、1.5(PI/2)が左、-1.5が右、3.14or-3.14が下。
        if(myA > 0){ // 上〜左〜下向きのとき
