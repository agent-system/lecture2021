...
int main(int argc, char **argv) {
  ...
  //const double k_vertical_thrust = 68.5;  // with this thrust, the drone lifts. 368
  const double k_vertical_thrust = 140;  // with this thrust, the drone lifts.
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    if (number_of_objects > 0 && time - time_p > 10.0) {
      target_altitude = objects[0].position[1] + altitude + 0.5;
      pitch_disturbance = 1.0;
      target_x = wb_gps_get_values(gps)[0] + objects[0].position[2] + 0.5;
      time_p = time;
      printf("target x: %f y: %f z: %f, pos: %f, %f, %f\n", objects[0].position[0], objects[0].position[1], objects[0].position[2], wb_gps_get_values(gps)[0], wb_gps_get_values(gps)[1], wb_gps_get_values(gps)[2]);
    }
    if (number_of_objects > 0) {
      if (objects[0].position[0] > 0.1) {
        yaw_disturbance = 1.0;
    ...
    printf("x: %f, %f\n", wb_gps_get_values(gps)[0] , target_x);
    if (wb_gps_get_values(gps)[0] < target_x) {
      wb_motor_set_velocity(front_left_motor, -80);//+
      wb_motor_set_velocity(front_right_motor, 80);//-
      wb_motor_set_velocity(rear_left_motor, -0);//-
      wb_motor_set_velocity(rear_right_motor, 0);//+
      wb_motor_get_position_sensor(rear_left_motor, 0);
      wb_motor_get_position_sensor(rear_right_motor, -0);
      wb_motor_set_position(rear_left_motor, 0);
      wb_motor_set_position(rear_right_motor, -0);
      //sokudo zero dato murikamo, pos sensor site chikakude tomarubeki
      break;
    }
