...
int main() {
  ...
  while (wb_robot_step(CONTROL_STEP) != -1) {
    ...
    if (init_flag) {
      for (int i = 0; i < 3; i++) {
        before_gps[i] = wb_gps_get_values(gps)[i];
      }
      init_flag = false;
    } else {
      for (int i = 0; i < 3; i++) {
        velocity[i] = 0.95 * velocity[i] +  0.05 * (wb_gps_get_values(gps)[i] - before_gps[i]);
        before_gps[i] = wb_gps_get_values(gps)[i];
      }
    }
    ...
    if (control == AUTO) {
      ...
      if (-2.5 < wb_gps_get_values(gps)[X] && wb_gps_get_values(gps)[X] < 2.5 && -1.5 < wb_gps_get_values(gps)[Z] && wb_gps_get_values(gps)[Z] < 1.5) {
        ...
        if (velocity[X] > 0) {
          ...
          //printf("kick the ball\n");
        //if (right_val < 0.6 || left_val < 0.6) {
        //  printf("atari, %f, %f\n", right_val, left_val);
        //  look_ball = true;
        //  fp = fopen("salamanderb.txt", "w");
        //  if (fp == NULL) {
        //    printf("hogee\n");
        //  }
        //  fprintf(fp, "%d %f %f %f\n", 1, wb_gps_get_values(gps)[X], wb_gps_get_values(gps)[Y], wb_gps_get_values(gps)[Z]);
        //  fclose(fp);
        //} else {
        //  printf("hazure, %f, %f\n", right_val, left_val);
        //  spine_offset = (right_val - left_val);
        ...
        //    int d1;
        //    float d2, d3, d4;
        //    fp = fopen("salamanderb.txt", "r");
        //    if (fp == NULL) {
        //      printf("hogee\n");
        //    }
        //    fscanf(fp, "%d %f %f %f", &d1, &d2, &d3, &d4);
        //    fclose(fp);
        //    if (d1 == 1) {
        //      if ((d2-wb_gps_get_values(gps)[X]) * velocity[Z] - (d4 - wb_gps_get_values(gps)[Z]) * velocity[X] > 0) {
        //        spine_offset += 0.2;
        //      } else {
        //        spine_offset -= 0.2;
        //      }
        ...
        spine_offset = (right_val - left_val);
        //printf("get away from the wall\n");
      }
      //printf("right_val: %f, left_val; %f\n", right_val, left_val);
    }
