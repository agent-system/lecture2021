...
int main() {
  ...
  WbDeviceTag receiver; //receive information from robot
  ...
  //2team 3player create 
  char team_name;   /* A or B*/
  char player_id; /* can be either '1', '2' or '3' */
  const char *name;
  name = wb_robot_get_name();
  team_name = name[0]; //'A' or 'B'
  player_id = name[1]; // '1', '2' or '3'
  ...
  //variable of salamander and ball status
  double salamanderX = 0, salamanderZ = 0, salamanderA = 0, ballX = 0, ballZ = 0;
  ...
  while (wb_robot_step(CONTROL_STEP) != -1) {
    while (wb_receiver_get_queue_length(receiver) > 0) {
      ...
      switch (team_name + player_id){
        case 'R' + '1':
          salamanderX = message[0]; salamanderZ = message[1], salamanderA = message[2];
        break;
        case 'R' + '2':
          salamanderX = message[3]; salamanderZ = message[4], salamanderA = message[5];
        break;
        case 'R' + '3':
          salamanderX = message[6]; salamanderZ = message[7], salamanderA = message[8];
        break;
        case 'B' + '1':
          salamanderX = message[9]; salamanderZ = message[10], salamanderA = message[11];
        break;
        case 'B' + '2':
          salamanderX = message[12]; salamanderZ = message[13], salamanderA = message[14];
        break;
        case 'B' + '3':
          salamanderX = message[15]; salamanderZ = message[16], salamanderA = message[17];
        break;
      }
      ballX = message[18], ballZ = message[19];
    ...
    if (control == AUTO) {
      if(salamanderX - ballX > 0) { // salamanderX is the higher than ball
