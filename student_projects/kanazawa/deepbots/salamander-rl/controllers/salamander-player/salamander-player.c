...
int main() {
  ...
  while (wb_robot_step(CONTROL_STEP) != -1) {
    ...
    while (wb_receiver_get_queue_length(receiver) > 0) {
      ...
      double sal_x,sal_z,sal_r,ball_x,ball_z,ball_r,tmp,del_x,del_z;
      const double keisuu = 0.2; /*0.1*/
      double def_val;
      
      printf(" packet num : %g,%g,%g,%g \n",packet[0],packet[1],packet[2],packet[3]);
      ...
      tmp = (del_x * -1) / (del_z * -1);
      ball_r = atan(tmp);
      if((del_z >0) && (del_x >0)) ball_r = ball_r - 3.14;
      if((del_z >0) && (del_x <0)) ball_r = ball_r + 3.14;
      printf("sal_r : %g, ball_r : %g, deg: %g, del_x: %g, del_z: %g \n", sal_r,ball_r,ball_r * 180.0 / 3.141592653589793 , del_x ,del_z);
	...
	/* /\* use camera to follow ball *\/ */
	/* spine_offset = 0.1; */
	/* /\* printf(" spine_offset : %f \n",spine_offset); *\/ */

	/* use receiver to follow ball */
	spine_offset = (sal_r - ball_r)*keisuu;

	/* if(((ball_r > 0.14) && (ball_r < 3.0)) || ((ball_r < -0.14) && (ball_r > -3.0)) ){ */
	/*   spine_offset = (sal_r - ball_r)*keisuu; */
	/* } */


	/* if((ball_r > 3.1) || (ball_r < -3.1)){ */
	/*   spine_offset = 1.5; */
	/*   printf("   kinnkyuu kaihi !!!! \n"); */
	/* } else { */
	/*   spine_offset = (sal_r - ball_r)*keisuu; */
	/* } */

	/* if(((sal_r - ball_r) > 3) || ((sal_r - ball_r) < -3)){ */
	/*   spine_offset = 1.5; */
	/*   printf("   kinnkyuu kaihi !!!! \n"); */
	/* } else { */
	/*   spine_offset = (sal_r - ball_r)*keisuu; */
	...
	if((sal_r - ball_r) > 3.14){
	  def_val = sal_r - (6.38+ball_r);
	} else {
	  def_val = sal_r - ball_r;
	}
	...
	if (locomotion == WALK) {
	  ...
	  /* printf("phase num : %f", SWIM_AMPL * ampl * sin(phase + 0 * (2 * M_PI / 6)) * ((0 + 5) / 10.0) + spine_offset); */
	  for (i = 0; i < 6; i++)
