...
Robot::Robot(const char *name) : _name(name), _controlStep(SIMULATION_STEP_DURATION),
				 _stepCount(0), _camera(0) {
  int i = 0;
 ...
 */
void Robot::computeWalkingPosition(double *motorsPosition, double t, double gait_freq, int gait_type,
				   int legId, double stride_length_factor, bool backward) {
  ...
  // compute angle A1
  double A1 = acos(((L1 + L2 * cos(A2)) * x - (-L2 * sin(A2)) * y) /
		   (pow(L1 + L2 * cos(A2), 2) + pow(-L2 * sin(A2), 2)));
...
void Robot::interactive_walk() {
  ...
  while (true) {
    ...
    if (display_info) {
      printf("gait_type:%d %s\n", gait_type, Robot::gait_name[gait_type % 7]);
