...
export BASE_IMAGE=jskrobotics/agent_system:robot_assembler_2021
...
docker build . --no-cache --file Dockerfile.webots-docker \
       --build-arg BASE_IMAGE=${BASE_IMAGE} \
       --build-arg WEBOTS_PACKAGE_PREFIX=_ubuntu-18.04 \
       --tag ${TEMP_IMAGE}

### not required because robot_assembler is already wrapped
#> ### wrap for xserver
...
docker build . --no-cache -f ../dockerfiles/Dockerfile.urdf2webots \
       --build-arg BASE_IMAGE=${TEMP_IMAGE} \
       --build-arg CATKIN_WS=/catkin_ws \
       --build-arg CATKIN_BUILD=robot_assembler \
       --tag jskrobotics/agent_system:webots_2021
